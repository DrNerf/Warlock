...
namespace UnityEngine.UI
{
	public class UICharacterSelect_List : MonoBehaviour {
		...
		{
			// Disable the separator on the last unit in the list
			UICharacterSelect_Unit[] units = this.gameObject.GetComponentsInChildren<UICharacterSelect_Unit>(false);
			
			for (int i = 0; i < units.Length; ++i)
			{
				units[i].ToggleSeparator(i < (units.Length - 1));
			}
		}
