...
namespace UnityEngine.UI
{
	...
	public class UIRealmSelect_Realm : Toggle {
		...
		[SerializeField] private float m_HighlighColorMultiplier = 1.45f;
		...
		[SerializeField] private Color m_CharactersNormalColor = Color.white;
		...
		protected override void Start()
		{
			...
			// Check if we have a toggle group
			if (this.group == null)
			{
				// Try getting in parent
				this.group = this.gameObject.GetComponentInParent<ToggleGroup>();
				
				// Check if the parent dont have a group else add one
				if (this.group == null)
			...
			// Get the characters text normal color for hover multiplication
			if (Application.isPlaying && this.m_CharactersText != null)
			{
				this.m_CharactersNormalColor = this.m_CharactersText.color;
			}
		}
		...
		protected void ResetToStartingState()
		{
			...
			// Reset the characters count text
		...
		protected void ApplyStatusState()
		{
			...
			{
				Color color = (this.m_CurrentStatus == Status.Online) ? this.m_StatusOnlineColor : this.m_StatusOfflineColor;
				
				if (this.m_IsHighlighted)
				{
					color = new Color(
						Mathf.Clamp((color.r * this.m_HighlighColorMultiplier), 0f, 1f),
						Mathf.Clamp((color.g * this.m_HighlighColorMultiplier), 0f, 1f),
						Mathf.Clamp((color.b * this.m_HighlighColorMultiplier), 0f, 1f),
						Mathf.Clamp((color.a * this.m_HighlighColorMultiplier), 0f, 1f)
					);
				}
				
				this.m_StatusText.text = ((this.m_CurrentStatus == Status.Online) ? this.m_StatusOnlineString : this.m_StatusOfflineString);
				this.m_StatusText.canvasRenderer.SetColor(color);
			}
		}
		...
		/// <summary>
		/// Applies the realm characters count state.
		/// </summary>
		protected void ApplyCharactersCountState()
		{
			if (this.m_CharactersText == null || !Application.isPlaying)
				return;
			
			Color color = this.m_CharactersNormalColor;
			...
			this.m_CharactersText.color = color;
		}
	}
}